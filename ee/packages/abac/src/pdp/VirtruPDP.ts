...
export class VirtruPDP implements IPolicyDecisionPoint {
	...
	private async checkAuthorizedAccess(token: string): Promise<void> {
		const config = this.client.getConfig();
		try {
			const response = await serverFetch(`${config.baseUrl}/authorization.AuthorizationService/GetDecisions`, {
				method: 'POST',
				timeout: HEALTH_CHECK_TIMEOUT,
	...
	private async getDecision(request: IGetDecisionRequest): Promise<Decision | undefined> {
		const result = await this.client.apiCall<IGetDecisionsResponse>('/authorization.AuthorizationService/GetDecisions', {
			...
			resourceAttributes: [
				{
					...
					attributeValueFqns: buildAttributeFqns(config.attributeNamespace, attributes),
				},
			],
			entityChains: [
				{
